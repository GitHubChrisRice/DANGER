...
#if defined(NDEBUG)
# error "Dangermoon cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3b2e0sd8762d21d5445d5ae00a0c1f18c480ec06ddbb5df9202e8c263ea6ebd03");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dangermoon: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dangermoon Signed Message:\n";
...
{
    // Dangermoon: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Dangermoon: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Dangermoon: 3 days
static const int64 nTargetSpacing = 2.5 * 60; // Dangermoon: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dangermoon: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dangermoon: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa60cf66ab8aaceabb64f60f6b08b03bc12706a64dd0e27255f5e9ad73b734c27");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = " 5/13/2021 - Fully Vaccinated People Can Stop Wearing Masks Indoors And Outdoors, CDC Says";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0430b7ccee3181a83c4a4fb5ccb1e9c0e94f0073f253e173c038e542e48cc28f3174d84d8e18fc04bdf875f3a456fab5e4f46636a90c2b564aac1f80ee89af51b4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1620939527;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085759641;

        if (fTestNet)
        {
            block.nTime    = 1620939563;
            block.nNonce   = 385999852;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x93e65e4a250db5d9fd662ae2c8f20215880e55555b03142594769ba13470dbe1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb8, 0xdc }; // Dangermoon: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DangermoonMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DangermoonMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DangermoonMiner(CWallet *pwallet)
{
    printf("DangermoonMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dangermoon-miner");
    ...
    try { loop {
        ...
        printf("Running DangermoonMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DangermoonMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DangermoonMiner, pwallet));
}
